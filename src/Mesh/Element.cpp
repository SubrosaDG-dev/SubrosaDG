...
inline void VolumeElementMesh<VolumeElementTrait>::getVolumeElementMesh(
  ...
  for (Isize i = 0; i < this->number_; i++) {
    ...
    physical.information_[gmsh_physical_index - 1].element_gmsh_tag_.emplace_back(gmsh_tag);
    ...
    physical.gmsh_tag_to_element_physical_information_[gmsh_tag].element_index_ = i;
...
template <typename AdjacencyElementTrait, Is1dElement VolumeElementTrait>
inline void getAdjacencyElementMeshSupplementalMap(
  ...
  for (Usize i = 0; i < all_adjacency_number; i++) {
    ...
    adjacency_element_mesh_supplemental_map[point_tag].adjacency_sequence_.emplace_back(
        parent_and_self_sequence.second);
    adjacency_element_mesh_supplemental_map[point_tag].parent_gmsh_type_.emplace_back(
        VolumeElementTrait::kGmshTypeNumber);
  }
}

template <typename AdjacencyElementTrait, Is2dElement VolumeElementTrait>
inline void getAdjacencyElementMeshSupplementalMap(
    std::unordered_map<Isize, AdjacencyElementMeshSupplemental<AdjacencyElementTrait>>&
        adjacency_element_mesh_supplemental_map) {
  std::vector<std::size_t> adjacency_node_tags;
  ...
  for (Usize i = 0; i < all_adjacency_number; i++) {
    ...
    adjacency_element_mesh_supplemental_map[edge_tag].parent_gmsh_tag_.emplace_back(
        element_tags[static_cast<Usize>(parent_and_self_sequence.first)]);
    adjacency_element_mesh_supplemental_map[edge_tag].adjacency_sequence_.emplace_back(parent_and_self_sequence.second);
    adjacency_element_mesh_supplemental_map[edge_tag].parent_gmsh_type_.emplace_back(
        VolumeElementTrait::kGmshTypeNumber);
  }
}

template <typename AdjacencyElementTrait, Is3dElement VolumeElementTrait>
inline void getAdjacencyElementMeshSupplementalMap(
    std::unordered_map<Isize, AdjacencyElementMeshSupplemental<AdjacencyElementTrait>>&
        adjacency_element_mesh_supplemental_map) {
  std::vector<std::size_t> adjacency_node_tags;
  ...
  for (Usize i = 0; i < all_adjacency_number; i++) {
    ...
    if (!adjacency_element_mesh_supplemental_map.contains(face_tag)) {
      ...
      adjacency_element_mesh_supplemental_map[face_tag].right_rotation_ = static_cast<Isize>(std::distance(
    ...
    adjacency_element_mesh_supplemental_map[face_tag].parent_gmsh_tag_.emplace_back(
        element_tags[static_cast<Usize>(parent_and_self_sequence.first)]);
    adjacency_element_mesh_supplemental_map[face_tag].adjacency_sequence_.emplace_back(parent_and_self_sequence.second);
    adjacency_element_mesh_supplemental_map[face_tag].parent_gmsh_type_.emplace_back(
        VolumeElementTrait::kGmshTypeNumber);
  }
}
...
inline void fixAdjacencyElementMeshSupplementalMap(
    const MeshPhysical& physical, std::unordered_map<Isize, AdjacencyElementMeshSupplemental<AdjacencyElementTrait>>&
                                      adjacency_element_mesh_supplemental_map) {
  for (Isize i = 0; i < physical.number_; i++) {
    ...
    adjacency_element_mesh_supplemental_map[node_tag.first].adjacency_sequence_.emplace_back(
        adjacency_element_mesh_supplemental_map[node_tag.second].adjacency_sequence_[0]);
    adjacency_element_mesh_supplemental_map[node_tag.first].parent_gmsh_type_.emplace_back(
        adjacency_element_mesh_supplemental_map[node_tag.second].parent_gmsh_type_[0]);
  ...
  for (Isize i = 0; i < physical.number_; i++) {
    ...
    for (const auto [entity_tag, entity_tag_master] : periodic_entity_tag_map) {
      ...
      for (Usize j = 0; j < element_tags_master.size(); j++) {
        ...
        adjacency_element_mesh_supplemental_map[element_tag_master].adjacency_sequence_.emplace_back(
            adjacency_element_mesh_supplemental_map[element_tag].adjacency_sequence_[0]);
        adjacency_element_mesh_supplemental_map[element_tag_master].parent_gmsh_type_.emplace_back(
            adjacency_element_mesh_supplemental_map[element_tag].parent_gmsh_type_[0]);
  ...
  for (Isize i = 0; i < physical.number_; i++) {
    ...
    for (const auto [entity_tag, entity_tag_master] : periodic_entity_tag_map) {
      ...
      for (Usize j = 0; j < element_tags_master.size(); j++) {
        ...
        adjacency_element_mesh_supplemental_map[element_tag_master].right_rotation_ = static_cast<Isize>(std::distance(
...
inline void AdjacencyElementMesh<AdjacencyElementTrait>::getAdjacencyElementInteriorMesh(
  ...
  for (Isize i = 0; i < this->interior_number_; i++) {
    ...
    const Isize gmsh_tag = static_cast<Isize>(max_tag) + i + 1;
    this->gmsh_tag_(i) = gmsh_tag;
    this->element_index_(i) = i;
    interior_gmsh_tag.emplace_back(gmsh_tag);
    for (Isize j = 0; j < AdjacencyElementTrait::kAllNodeNumber; j++) {
      this->node_coordinate_(i).col(j) =
          node_coordinate.col(adjacency_element_mesh_supplemental.node_tag_[static_cast<Usize>(j)] - 1);
      this->node_tag_(i)(j) = adjacency_element_mesh_supplemental.node_tag_[static_cast<Usize>(j)];
      interior_node_tag.emplace_back(adjacency_element_mesh_supplemental.node_tag_[static_cast<Usize>(j)]);
    }
    physical.gmsh_tag_to_element_physical_information_[gmsh_tag].element_index_ = i;
    this->adjacency_right_rotation_(i) = adjacency_element_mesh_supplemental.right_rotation_;
    this->left_parent_index_each_type_(i) =
        physical.gmsh_tag_to_element_physical_information_.at(adjacency_element_mesh_supplemental.parent_gmsh_tag_[0])
            .element_index_;
    this->right_parent_index_each_type_(i) =
        physical.gmsh_tag_to_element_physical_information_.at(adjacency_element_mesh_supplemental.parent_gmsh_tag_[1])
            .element_index_;
    this->adjacency_sequence_in_left_parent_(i) = adjacency_element_mesh_supplemental.adjacency_sequence_[0];
    this->adjacency_sequence_in_right_parent_(i) = adjacency_element_mesh_supplemental.adjacency_sequence_[1];
    this->left_parent_gmsh_type_number_(i) = adjacency_element_mesh_supplemental.parent_gmsh_type_[0];
    this->right_parent_gmsh_type_number_(i) = adjacency_element_mesh_supplemental.parent_gmsh_type_[1];
  }
...
inline void AdjacencyElementMesh<AdjacencyElementTrait>::getAdjacencyElementBoundaryMesh(
  ...
  for (Isize i = this->interior_number_; i < this->number_; i++) {
    const AdjacencyElementMeshSupplemental<AdjacencyElementTrait>& adjacency_element_mesh_supplemental =
    ...
    try {
      this->gmsh_tag_(i) = node_tag_element_map.at(node_tag);
    ...
    const Isize gmsh_tag = this->gmsh_tag_(i);
    const auto gmsh_physical_index =
        static_cast<Usize>(physical.gmsh_tag_to_element_physical_information_.at(gmsh_tag).gmsh_physical_index_);
    this->gmsh_physical_index_(i) = static_cast<Isize>(gmsh_physical_index);
    this->boundary_condition_type_(i) = physical.information_[gmsh_physical_index - 1].boundary_condition_type_;
    this->element_index_(i) = i;
    physical.information_[gmsh_physical_index - 1].element_number_++;
...
inline void AdjacencyElementMesh<AdjacencyElementTrait>::getAdjacencyElementMesh(
  ...
  if constexpr (AdjacencyElementTrait::kElementType == ElementEnum::Point) {
    getAdjacencyElementMeshSupplementalMap<AdjacencyElementTrait,
                                           VolumeLineTrait<AdjacencyElementTrait::kPolynomialOrder>>(
        adjacency_element_mesh_supplemental_map);
  } else if constexpr (AdjacencyElementTrait::kElementType == ElementEnum::Line) {
    if constexpr (HasTriangle<MeshModelType>) {
      getAdjacencyElementMeshSupplementalMap<AdjacencyElementTrait,
                                             VolumeTriangleTrait<AdjacencyElementTrait::kPolynomialOrder>>(
          adjacency_element_mesh_supplemental_map);
    }
    if constexpr (HasQuadrangle<MeshModelType>) {
      getAdjacencyElementMeshSupplementalMap<AdjacencyElementTrait,
                                             VolumeQuadrangleTrait<AdjacencyElementTrait::kPolynomialOrder>>(
          adjacency_element_mesh_supplemental_map);
    }
  } else if constexpr (AdjacencyElementTrait::kElementType == ElementEnum::Triangle) {
    if constexpr (HasTetrahedron<MeshModelType>) {
      getAdjacencyElementMeshSupplementalMap<AdjacencyElementTrait,
                                             VolumeTetrahedronTrait<AdjacencyElementTrait::kPolynomialOrder>>(
          adjacency_element_mesh_supplemental_map);
    }
    if constexpr (HasPyramid<MeshModelType>) {
      getAdjacencyElementMeshSupplementalMap<AdjacencyElementTrait,
                                             VolumePyramidTrait<AdjacencyElementTrait::kPolynomialOrder>>(
          adjacency_element_mesh_supplemental_map);
    }
  } else if constexpr (AdjacencyElementTrait::kElementType == ElementEnum::Quadrangle) {
    if constexpr (HasPyramid<MeshModelType>) {
    ...
    if constexpr (HasHexahedron<MeshModelType>) {
      getAdjacencyElementMeshSupplementalMap<AdjacencyElementTrait,
                                             VolumeHexahedronTrait<AdjacencyElementTrait::kPolynomialOrder>>(
          adjacency_element_mesh_supplemental_map);
    }
  }
  fixAdjacencyElementMeshSupplementalMap<AdjacencyElementTrait>(physical, adjacency_element_mesh_supplemental_map);
  ...
  this->number_ = this->interior_number_ + this->boundary_number_;
  this->gmsh_tag_.resize(this->number_);
  this->gmsh_physical_index_.resize(this->number_);
  this->element_index_.resize(this->number_);
  this->node_tag_.resize(this->number_);
  ...
  this->getAdjacencyElementInteriorMesh(node_coordinate, physical, interior_tag,
                                        adjacency_element_mesh_supplemental_map);
