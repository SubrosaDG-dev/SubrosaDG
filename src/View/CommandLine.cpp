...
struct CommandLine {
  ...
  std::string getVariableList() {
    if constexpr (SimulationControl::kEquationModel == EquationModelEnum::CompressibleEuler ||
                  SimulationControl::kEquationModel == EquationModelEnum::CompressibleNS) {
    ...
    if constexpr (SimulationControl::kEquationModel == EquationModelEnum::IncompressibleEuler ||
                  SimulationControl::kEquationModel == EquationModelEnum::IncompressibleNS) {
      if constexpr (SimulationControl::kDimension == 1) {
        return std::format(R"(|{:^13}|{:^13}|{:^13}|{:^13}|)", "Time", "rho", "rho*u", "rho*e");
      } else if constexpr (SimulationControl::kDimension == 2) {
        return std::format(R"(|{:^13}|{:^13}|{:^13}|{:^13}|{:^13}|)", "Time", "rho", "rho*u", "rho*v", "rho*e");
  ...
  std::string getLineInformation(const Eigen::Vector<Real, SimulationControl::kConservedVariableNumber>& error,
                                 const Real time_value) {
    if constexpr (SimulationControl::kDimension == 1) {
  ...
  void initializeSolver(const TimeIntegration<SimulationControl>& time_integration, std::fstream& error_finout,
                        const int error_output_interval) {
    ...
    for (Usize i = 0; i < static_cast<Usize>(this->line_number_); i++) {
      error_string += this->getLineInformation(this->error_deque_[i], this->time_value_deque_[i]) + '\n';
    }
    if (this->is_open_ == CommandLineEnum::Open) {
      ...
      this->solver_progress_bar_.initialize(time_integration.iteration_start_, time_integration.iteration_end_,
                                            error_output_interval, this->line_number_ + 2);
    }
  }
  ...
  void initializeView(const int iteration_number) {
    if (this->is_open_ == CommandLineEnum::Open) {
      std::cout << '\n';
      this->view_progress_bar_.restart();
      this->view_progress_bar_.initialize(0, iteration_number);
    }
  }
  ...
  void printInformation() {
    if (this->is_open_ == CommandLineEnum::Open) {
      ...
      information << "\n";
      information << R"(#)" << R"(#######################################################)" << R"(#)" << '\n'
                  << R"(#)" << BOLDRED << R"(   ____        _                        )" << BOLDBLUE
                  << R"( ____   ____   )" << RESET << R"(#)" << '\n'
                  << R"(#)" << BOLDRED << R"(  / ___| _   _| |__  _ __ ___  ___  __ _)" << BOLDBLUE
                  << R"(|  _ \ / ___|  )" << RESET << R"(#)" << '\n'
                  << R"(#)" << BOLDRED << R"(  \___ \| | | | '_ \| '__/ _ \/ __|/ _` )" << BOLDMAGENTA << R"(|)"
                  << BOLDBLUE << R"( | | | |  _   )" << RESET << R"(#)" << '\n'
                  << R"(#)" << BOLDRED << R"(   ___) | |_| | |_) | | | (_) \__ \ (_| )" << BOLDMAGENTA << R"(|)"
                  << BOLDBLUE << R"( |_| | |_| |  )" << RESET << R"(#)" << '\n'
                  << R"(#)" << BOLDRED << R"(  |____/ \__,_|_.__/|_|  \___/|___/\__,_)" << BOLDMAGENTA << R"(|)"
                  << BOLDBLUE << R"(____/ \____|  )" << RESET << R"(#)" << '\n'
                  << R"(#)" << R"(                                                       )" << R"(#)" << '\n'
                  << R"(#)" << R"(#######################################################)" << R"(#)" << '\n';
      ...
      information << std::format("Memory Size: {:.2f} GiB",
                                 static_cast<Real>(kDevice.get_info<sycl::info::device::global_mem_size>()) /
                                     std::pow(1024.0_r, 3))
                  << '\n';
  ...
  CommandLine(const CommandLineEnum command_line_type) {
    this->is_open_ = command_line_type;
    if (this->is_open_ == CommandLineEnum::Open) {
      std::stringstream information;
