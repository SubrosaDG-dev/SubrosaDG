...
struct PhysicalModelData {
  ...
  // TransportModel
  static constexpr double kReynoldsNumber = 40.0;
  static constexpr double kDynamicViscosity = 1.0 * 0.5 * 2.0 / kReynoldsNumber;
...
//   gmsh::model::geo::mesh::setTransfiniteCurve(1, 128 + 1);
//   gmsh::model::geo::mesh::setTransfiniteCurve(2, 128 + 1);
//   gmsh::model::geo::mesh::setTransfiniteCurve(3, 128 + 1);
//   gmsh::model::geo::mesh::setTransfiniteCurve(4, 128 + 1);
...
void generateMesh(const std::filesystem::path& mesh_file_path) {
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 2; j++) {
      gmsh::model::geo::mesh::setTransfiniteCurve(line_x_tag(j, i), 16 + 1);
      gmsh::model::geo::mesh::setTransfiniteCurve(line_y_tag(j, i), 16 + 1);
    }
  }
  for (int i = 0; i < 4; i++) {
    gmsh::model::geo::mesh::setTransfiniteSurface(plane_surface_tag(i));
  }
  gmsh::model::geo::mesh::setRecombine(2, plane_surface_tag(0));
  gmsh::model::geo::mesh::setRecombine(2, plane_surface_tag(3));
  gmsh::model::geo::synchronize();
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 2; j++) {
      if (i != 1) {
