/**
 * @file periodic_3d_euler.cpp
 * @brief The source file for SubrosaDG example periodic_3d_euler.
 *
...
inline const std::string kExampleName{"periodic_3d_euler"};
...
int main(int argc, char* argv[]) {
  ...
  system.setMesh(kExampleDirectory / "periodic_3d_euler.msh", generateMesh);
  system.addInitialCondition([](const Eigen::Vector<SubrosaDG::Real, SimulationControl::kDimension>& coordinate)
    ...
    return Eigen::Vector<SubrosaDG::Real, SimulationControl::kPrimitiveVariableNumber>{
        1.0 + 0.2 * std::sin(SubrosaDG::kPi * (coordinate.x() + coordinate.y() + coordinate.z())), 0.5, 0.3, 0.2,
        1.4 / (1.0 + 0.2 * std::sin(SubrosaDG::kPi * (coordinate.x() + coordinate.y() + coordinate.z())))};
  });
...
void generateMesh(const std::filesystem::path& mesh_file_path) {
  ...
  Eigen::Vector<double, 4> point_coordinate;
  // clang-format off
